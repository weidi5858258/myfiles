...
#define LOG_TAG "alexander-AudioTrack"
...
static struct audio_track_fields_t javaAudioTrackFields;
...
sp<AudioTrack> android_media_AudioTrack_getAudioTrack(JNIEnv *env, jobject audioTrackObj) {
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_getAudioTrack() PID: %d", pid);
    return getAudioTrack(env, audioTrackObj);
}

// ----------------------------------------------------------------------------
static jint
android_media_AudioTrack_setup(JNIEnv *env, jobject thiz,
                               jobject weak_this,
                               jobject jaa,
                               jintArray jSampleRate,
                               jint channelPositionMask,
    ...
    ALOGV("sampleRates=%p, channel mask=%x, index mask=%x, audioFormat(Java)=%d, buffSize=%d"
          ...
          nativeAudioTrack);
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_setup() PID: %d", pid);
    ALOGD("android_media_AudioTrack_setup() "
          "sampleRates=%p, channel mask=%x, "
          "index mask=%x, audioFormat(Java)=%d, "
          "buffSize=%d, memoryMode=%d, offload=%d",
          jSampleRate, channelPositionMask,
          channelIndexMask, audioFormat,
          buffSizeInBytes, memoryMode, offload);
...
android_media_AudioTrack_start(JNIEnv *env, jobject thiz) {
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_start() PID: %d", pid);
...
android_media_AudioTrack_stop(JNIEnv *env, jobject thiz) {
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_stop() PID: %d", pid);
    sp<AudioTrack> lpTrack = getAudioTrack(env, thiz);
    if (lpTrack == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException",
...
android_media_AudioTrack_pause(JNIEnv *env, jobject thiz) {
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_pause() PID: %d", pid);
    sp<AudioTrack> lpTrack = getAudioTrack(env, thiz);
    if (lpTrack == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException",
...
android_media_AudioTrack_flush(JNIEnv *env, jobject thiz) {
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_flush() PID: %d", pid);
    sp<AudioTrack> lpTrack = getAudioTrack(env, thiz);
    if (lpTrack == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException",
...
android_media_AudioTrack_set_volume(JNIEnv *env, jobject thiz, jfloat leftVol, jfloat rightVol) {
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_set_volume() PID: %d", pid);
    sp<AudioTrack> lpTrack = getAudioTrack(env, thiz);
    if (lpTrack == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException",
...
static void android_media_AudioTrack_release(JNIEnv *env, jobject thiz) {
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_release() PID: %d", pid);
...
static void android_media_AudioTrack_finalize(JNIEnv *env, jobject thiz) {
    pid_t pid = getpid();
    ALOGD("android_media_AudioTrack_finalize() PID: %d", pid);
...
int register_android_media_AudioTrack(JNIEnv *env) {
    pid_t pid = getpid();
    ALOGD("register_android_media_AudioTrack() PID: %d", pid);
    // must be first
