...
// ----------------------------------------------------------------------------
    ...
    AudioFlinger::TrackHandle::TrackHandle(const sp<AudioFlinger::PlaybackThread::Track> &track) :
            ...
            mTrack(track) {
        pid_t pid = getpid();
        ALOGD("TrackHandle::TrackHandle()  created   %p PID: %d", this, pid);
    }
    ...
    AudioFlinger::TrackHandle::~TrackHandle() {
        ...
        mTrack->destroy();
        pid_t pid = getpid();
        ALOGD("TrackHandle::~TrackHandle() destroyed %p PID: %d", this, pid);
    }

    sp<IMemory> AudioFlinger::TrackHandle::getCblk() const {
        pid_t pid = getpid();
        ALOGD("TrackHandle::getCblk() %p PID: %d", this, pid);
        return mTrack->getCblk();
    }

    status_t AudioFlinger::TrackHandle::start() {
        pid_t pid = getpid();
        ALOGD("TrackHandle::start() %p PID: %d", this, pid);
        return mTrack->start();
    }

    void AudioFlinger::TrackHandle::stop() {
        pid_t pid = getpid();
        ALOGD("TrackHandle::stop() %p PID: %d", this, pid);
        mTrack->stop();
    }

    void AudioFlinger::TrackHandle::flush() {
        pid_t pid = getpid();
        ALOGD("TrackHandle::flush() %p PID: %d", this, pid);
        mTrack->flush();
    }

    void AudioFlinger::TrackHandle::pause() {
        pid_t pid = getpid();
        ALOGD("TrackHandle::pause() %p PID: %d", this, pid);
        mTrack->pause();
    }

    status_t AudioFlinger::TrackHandle::attachAuxEffect(int EffectId) {
        pid_t pid = getpid();
        ALOGD("TrackHandle::attachAuxEffect() %p PID: %d", this, pid);
        ...
        for (int i = ExtendedTimestamp::LOCATION_SERVER; i < ExtendedTimestamp::LOCATION_MAX; ++i) {
            if (local.mTimeNs[i] != 0) {
                ...
                const int64_t relativeTrackFrames = relativeServerFrames
                                                    * mSampleRate /
                                                    halSampleRate; // TODO: potential computation overflow
