...
Exclusion Standard (F</robots.txt>).  Wget can be instructed to
...
the default.  For instance, using C<follow_ftp = on> in
F<.wgetrc> makes Wget I<follow> FTP links by default, and
...
=item B<--report-speed=>I<type>

Output bandwidth as I<type>.  The only accepted value is B<bits>.
...
Read URLs from a local or external I<file>.  If B<-> is
specified as I<file>, URLs are read from the standard input.  
(Use B<./-> to read from a file literally named B<->.)

If this function is used, no URLs need be present on the command
line.  If there are URLs both on the command line and in an input
file, those on the command lines will be the first ones to be
retrieved.  If B<--force-html> is not specified, then I<file>
...
treated as B<html> if the Content-Type matches B<text/html>.
Furthermore, the I<file>'s location will be implicitly used as base
href if none was specified.


=item B<--input-metalink=>I<file>

Downloads files covered in local Metalink I<file>. Metalink version 3
and 4 are supported.
...
=item B<--metalink-index=>I<number>

Set the Metalink B<application/metalink4+xml> metaurl ordinal
NUMBER. From 1 to the total number of "application/metalink4+xml"
available.  Specify 0 or B<inf> to choose the first good one.
Metaurls, such as those from a B<--metalink-over-http>, may have
been sorted by priority key's value; keep this in mind to choose the
right NUMBER.


=item B<--preferred-location>
...
Resolves relative links using I<URL> as the point of reference,
when reading links from an HTML file specified via the
B<-i>/B<--input-file> option (together with
B<--force-html>, or when the input file was fetched remotely from
a server describing it as HTML). This is equivalent to the
presence of a C<BASE> tag in the HTML input file, with
I<URL> as the value for the C<href> attribute.

For instance, if you specify B<http://foo/bar/a.html> for
I<URL>, and Wget reads B<../baz/b.html> from the input file, it
...
=item B<--bind-dns-address=>I<ADDRESS>
...
with B<--dns-servers> is your friend.
I<ADDRESS> must be specified either as IPv4 or IPv6 address.
Wget needs to be built with libcares for this option to be available.


=item B<--dns-servers=>I<ADDRESSES>

[libcares only]
The given address(es) override the standard nameserver
addresses,  e.g. as configured in /etc/resolv.conf.
I<ADDRESSES> may be specified either as IPv4 or IPv6 addresses,
comma-separated.
Wget needs to be built with libcares for this option to be available.
...
Set number of tries to I<number>. Specify 0 or B<inf> for
...
A combination with B<-nc> is only accepted if the given output
file does not exist.
...
B<-k> can be used only when the output is a regular file.
...
When running Wget without B<-N>, B<-nc>, B<-r>, or
B<-p>, downloading the same file in the same directory will result
in the original copy of I<file> being preserved and the second copy
being named I<file>B<.1>.  If that file is downloaded yet
again, the third copy will be named I<file>B<.2>, and so on.
(This is also the behavior with B<-nd>, even if B<-r> or
B<-p> are in effect.)  When B<-nc> is specified, this behavior
is suppressed, and Wget will refuse to download newer copies of
I<file>.  Therefore, "C<no-clobber>" is actually a
...
B<-nd>, or B<-nc>, re-downloading a file will result in the
new copy simply overwriting the old.  Adding B<-nc> will prevent
...
A combination with B<-O>/B<--output-document> is only accepted
...
=item B<--backups=>I<backups>

Before (over)writing a file, back up an existing file by adding a
B<.1> suffix (B<_1> on VMS) to the file name.  Such backup
files are rotated to B<.2>, B<.3>, and so on, up to
I<backups> (and lost beyond that).


=item B<--no-netrc>

Do not try to obtain credentials from F<.netrc> file. By default
F<.netrc> file is searched for credentials in case none have been
...
Beginning with Wget 1.7, if you use B<-c> on a file which is of
...
B<--start-pos> has higher precedence over B<--continue>.  When
B<--start-pos> and B<--continue> are both specified, wget will emit a
warning then proceed as if B<--continue> was absent.
...
B<--progress=>I<type>B<:>I<parameter1>B<:>I<parameter2>.

When using the dotted retrieval, you may set the I<style> by
...
lines).  The C<mega> style is suitable for downloading large
files---each dot represents 64K retrieved, there are eight dots in a
cluster, and 48 dots on each line (so each line contains 3M).
If C<mega> is not enough then you can use the C<giga>
...
parameter as B<--progress=bar:force>.
...
use B<--progress=bar:force:noscroll>.
...
any other verbosity modes like B<--no-verbose> or B<--quiet>.  This
...
This option will also force the progress bar to be printed to F<stderr> when
used alongside the B<--logfile> option.
...
file, up to the maximum number of I<seconds> you specify.
...
specify B<wget -Q10k https://example.com/ls-lR.gz>, all of the
...
=item B<--restrict-file-names=>I<modes>

Change which characters found in remote URLs must be escaped during
generation of local filenames.  Characters that are I<restricted>
...
The I<modes> are a comma-separated set of text values. The
acceptable values are B<unix>, B<windows>, B<nocontrol>,
B<ascii>, B<lowercase>, and B<uppercase>. The values
B<unix> and B<windows> are mutually exclusive (one will
override the other), as are B<lowercase> and
B<uppercase>. Those last are special cases, as they do not change
...
When "unix" is specified, Wget escapes the character B</> and
the control characters in the ranges 0--31 and 128--159.  This is the
default on Unix-like operating systems.

When "windows" is given, Wget escapes the characters B<\>,
B<|>, B</>, B<:>, B<?>, B<">, B<*>, B<E<lt>>,
...
If you specify B<nocontrol>, then the escaping of the control
...
The B<ascii> mode is used to specify that any bytes whose values
...
=item B<--prefer-family=none/IPv4/IPv6>
...
You can set the default command for use-askpass in the F<.wgetrc>.  That
...
Turn off internationalized URI (IRI) support. Use B<--iri> to
turn it on. IRI support is activated by default.

You can set the default state of IRI support using the C<iri>
command in F<.wgetrc>. That setting may be overridden from the
command line.


=item B<--local-encoding=>I<encoding>
...
Wget use the function C<nl_langinfo()> and then the C<CHARSET>
environment variable to get the locale. If it fails, ASCII is used.

You can set the default local encoding using the C<local_encoding>
command in F<.wgetrc>. That setting may be overridden from the
command line.


=item B<--remote-encoding=>I<encoding>


Force Wget to use I<encoding> as the default remote server encoding.
...
For HTTP, remote encoding can be found in HTTP C<Content-Type>
header and in HTML C<Content-Type http-equiv> meta tag.

You can set the default encoding using the C<remoteencoding>
command in F<.wgetrc>. That setting may be overridden from the
command line.
...
=item B<--default-page=>I<name>

Use I<name> as the default file name when it isn't known (i.e., for
URLs that end in a slash), instead of F<index.html>.


=item B<-E>


=item B<--adjust-extension>
...
B<text/html> or B<application/xhtml+xml>.

As of version 1.12, Wget will also ensure that any downloaded files of
type B<text/css> end in the suffix B<.css>, and the option was
renamed from B<--html-extension>, to better reflect its new
...
a C<Content-Encoding> of B<br>, B<compress>, B<deflate>
or B<gzip> end in the suffix B<.br>, B<.Z>, B<.zlib>
and B<.gz> respectively.
...
use the B<--use-askpass> or store them in F<.wgetrc> or F<.netrc>,
and make sure to protect those files from other users with C<chmod>.  If
...
=item C<Mozilla and Netscape 6.x.>
...
=item B<--compression=>I<type>

Choose the type of compression to be used.  Legal values are
B<auto>, B<gzip> and B<none>.

If B<auto> or B<gzip> are specified, Wget asks the server to
compress the file using the gzip compression format. If the server
compresses the file and responds with the C<Content-Encoding>
header field set appropriately, the file will be decompressed
automatically.

If B<none> is specified, wget will not ask the server to compress
...
please report any bugs to C<bug-wget@gnu.org>.
...
data in the request body.  B<--post-data> sends I<string> as
data, whereas B<--post-file> sends the contents of I<file>.
Other than that, they work in exactly the same way. In particular,
they I<both> expect content of the form C<key1=value1&key2=value2>,
...
accepts its content from a file. In particular, B<--post-file> is
I<not> for transmitting files as form attachments: those must
appear as C<key=value> data (with appropriate percent-coding) just
like everything else. Wget does not currently support
C<multipart/form-data> for transmitting POST data; only
C<application/x-www-form-urlencoded>. Only one of
B<--post-data> and B<--post-file> should be specified.

Please note that wget does not require the content to be of the form
C<key1=value1&key2=value2>, and neither does it test for it. Wget will
...
When sending a POST request using the B<--post-file> option, Wget treats
...
=item B<--method=>I<HTTP-Method>

For the purpose of RESTful scripting, Wget allows sending of other HTTP Methods
without the need to explicitly set them using B<--header=Header-Line>.
Wget will use whatever string is passed to it after B<--method> as the HTTP
Method to the server.


=item B<--body-data=>I<Data-String>


=item B<--body-file=>I<Data-File>

Must be set when additional data needs to be sent to the server along with the
Method specified using B<--method>.  B<--body-data> sends I<string> as
data, whereas B<--body-file> sends the contents of I<file>.  Other than that,
they work in exactly the same way.

Currently, B<--body-file> is I<not> for transmitting files as a whole.
Wget does not currently support C<multipart/form-data> for transmitting data;
only C<application/x-www-form-urlencoded>. In the future, this may be changed
so that wget sends the B<--body-file> as a complete file instead of sending its
contents to the server. Please be aware that Wget needs to know the contents of
BODY Data in advance, and hence the argument to B<--body-file> should be a
regular file. See B<--post-file> for a more detailed explanation.
Only one of B<--body-data> and B<--body-file> should be specified.
...
request method should I<not> change.  Another exception is when
the method is set to C<POST>, in which case the redirection rules
specified under B<--post-data> are followed.
...
When combined with B<--metalink-over-http> and B<--trust-server-names>,
a B<Content-Type: application/metalink4+xml> file is named using the
C<Content-Disposition> filename field, if available.


=item B<--content-on-error>
...
=item B<--retry-on-http-error=>I<code[,code,...]>
...
B<SSLv2>, B<SSLv3>, B<TLSv1>, B<TLSv1_1>, B<TLSv1_2>
and B<PFS>.  If B<auto> is used, the SSL library is given the
...
Specifying B<SSLv2>, B<SSLv3>, B<TLSv1>, B<TLSv1_1>,
B<TLSv1_2> or B<TLSv1_3> forces the use of the corresponding
...
Specifying B<PFS> enforces the use of the so-called Perfect Forward
...
=item B<--crl-file=>I<file>

Specifies a CRL file in I<file>.  This is needed for certificates
that have been revocated by the CAs.


=item B<--pinnedpubkey=file/hashes>
...
pseudo-random number generator on systems without F</dev/urandom>.
...
in C<$RANDFILE> or, if that is unset, in F<$HOME/.rnd>.
...
systems that support F</dev/urandom>.
...
consequence, Wget would ignore all the C<Strict-Transport-Security>
headers, and would not enforce any existing HSTS policy.


=item B<--hsts-file=>I<file>

By default, Wget stores its HSTS database in F<~/.wget-hsts>.
You can use B<--hsts-file> to override this. Wget will use
...
(ie. a site that has issued a C<Strict-Transport-Security> header and that
therefore has specified a concrete HSTS policy to be applied). Lines starting with
a dash (C<#>) are ignored by Wget. Please note that in spite of this convenient
...
C<E<lt>hostnameE<gt> SP [E<lt>portE<gt>] SP E<lt>include subdomainsE<gt> SP E<lt>createdE<gt> SP E<lt>max-ageE<gt>>

The I<hostname> and I<port> fields indicate the hostname and port to which
the given HSTS policy applies. The I<port> field may be zero, and it will, in
...
the hostname will be evaluated). When I<port> is different to zero, both the
...
The Wget testsuite (in F<testenv/>) creates HSTS databases with explicit ports
...
in production environments and I<port> will typically be zero. The last three fields
do what they are expected to. The field I<include_subdomains> can either be C<1>
or C<0> and it signals whether the subdomains of the target domain should be
part of the given HSTS policy as well. The I<created> and I<max-age> fields
...
measured in seconds elapsed since the timestamp stored in I<created>. Once that time
...
entries. If no HSTS entries were generated (no C<Strict-Transport-Security> headers
were sent by any of the servers) then no file will be created, not even an empty one. This
behaviour applies to the default database file (F<~/.wget-hsts>) as well: it will not be
...
=item B<--warc-file=>I<file>

Use I<file> as the destination WARC file.


=item B<--warc-header=>I<string>

Use I<string> into as the warcinfo record.


=item B<--warc-max-size=>I<size>

Set the maximum size of the WARC files to I<size>.
...
=item B<--warc-dedup=>I<file>

Do not store records listed in this CDX file.


=item B<--no-warc-compression>

Do not compress WARC files with GZIP.


=item B<--no-warc-digests>

Do not calculate SHA1 digests.


=item B<--no-warc-keep-log>

Do not store the log file in a WARC record.


=item B<--warc-tempdir=>I<dir>
...
an C<AUTH TLS> command: it assumes the server speaks FTPS and directly starts an
SSL/TLS connection. If the attempt is successful, the session continues just like
regular FTPS (C<PBSZ> and C<PROT> are sent, etc.).
Implicit FTPS is no longer a requirement for FTPS implementations, and thus
many servers may not support it. If B<--ftps-implicit> is passed and no explicit
...
This is what B<--no-ftps-resume-ssl> is for.


=item B<--ftps-clear-data-connection>

All the data connections will be in plain text. Only the control connection will be
under SSL/TLS. Wget will send a C<PROT C> command to achieve this, which must be
approved by the server.


=item B<--ftps-fallback-to-ftp>
...
If a server does not successfully reply to the initial C<AUTH TLS> command, or in the
...
Specify recursion maximum depth level I<depth>.
...
It works particularly well in conjunction with B<--adjust-extension>, although
this coupling is not enforced. It proves useful to populate Internet caches
with files downloaded from different hosts.

Example: if some link points to F<//foo.com/bar.cgi?xyz> with
B<--adjust-extension> asserted and its local destination is intended to be
F<./foo.com/bar.cgi?xyz.css>, then the link would be converted to
F<//foo.com/bar.cgi?xyz.css>. Note that only the filename part has been
...
your shell from expanding it, like in B<-A "*.mp3"> or B<-A '*.mp3'>.


=item B<--accept-regex> I<urlregex>


=item B<--reject-regex> I<urlregex>

Specify a regular expression to accept or reject the complete URL.


=item B<--regex-type> I<regextype>
...
If set, the B<http_proxy> and B<https_proxy> variables should
...
connections.  It is quite common that B<http_proxy> and
B<ftp_proxy> are set to the same URL.
...
B<no_proxy> is B<.mit.edu>, proxy will not be used to retrieve
documents from MIT.
...
E<lt>B<https://savannah.gnu.org/bugs/?func=additem&group=wget>E<gt>) or to our
mailing list E<lt>B<bug-wget@gnu.org>E<gt>.

Visit E<lt>B<https://lists.gnu.org/mailman/listinfo/bug-wget>E<gt> to
...
Originally written by Hrvoje Nikšić E<lt>hniksic@xemacs.orgE<gt>.

=head1 COPYRIGHT

Copyright (c) 1996-2011, 2015, 2018 Free Software Foundation,
Inc.
