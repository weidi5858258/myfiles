...
gethttp(const struct url *u, struct url *original_url, struct http_stat *hstat,
  ...
  bool metalink = !!(*dt & METALINK_METADATA);
    ...
    hstat->rderrmsg = NULL;
    hstat->newloc = NULL;
    xfree (hstat->remote_time);
    hstat->error = NULL;
    hstat->message = NULL;
    hstat->local_encoding = ENC_NONE;
    hstat->remote_encoding = ENC_NONE;
    ...
    *dt &= ~RETROKF;
    ...
    if (global_options.cookies) {
        ...
        for (scpos = 0;
             ...
             ++scpos) {
            char *set_cookie;
            BOUNDED_TO_ALLOCA (scbeg, scend, set_cookie);
  ...
  if (metalink)
    ...
    if (statcode == HTTP_STATUS_UNAUTHORIZED) {
        ...
        if (warc_enabled) {
            ...
            _err = read_response_body(hstat, sock, NULL, contlen, 0,
            ...
            if (_err != RETRFINISHED || hstat->res < 0) {
                CLOSE_INVALIDATE (sock);
                retval = _err;
                goto cleanup;
            } else
    ...
    {
        uerr_t ret = check_file_output(u, hstat, resp, hdrval, sizeof hdrval);
        if (ret != RETROK) {
            retval = ret;
            goto cleanup;
        }
    ...
    hstat->statcode = statcode;
    if (statcode == -1)
        hstat->error = xstrdup(_("Malformed status line"));
    else if (!*message)
        hstat->error = xstrdup(_("(no description)"));
    else
    ...
    hstat->newloc = resp_header_strdup(resp, "Location");
    hstat->remote_time = resp_header_strdup(resp, "Last-Modified");
    if (!hstat->remote_time) // now look for the Wayback Machine's timestamp
        hstat->remote_time = resp_header_strdup(resp, "X-Archive-Orig-last-modified");
    ...
    if (resp_header_copy(resp, "Content-Encoding", hdrval, sizeof(hdrval))) {
        hstat->local_encoding = ENC_INVALID;

        switch (hdrval[0]) {
            case 'b':
            case 'B':
                if (0 == c_strcasecmp(hdrval, "br"))
                    hstat->local_encoding = ENC_BROTLI;
                break;
            case 'c':
            case 'C':
                if (0 == c_strcasecmp(hdrval, "compress"))
                    hstat->local_encoding = ENC_COMPRESS;
                break;
            case 'd':
            case 'D':
                if (0 == c_strcasecmp(hdrval, "deflate"))
                    hstat->local_encoding = ENC_DEFLATE;
                break;
            case 'g':
            case 'G':
                if (0 == c_strcasecmp(hdrval, "gzip"))
                    hstat->local_encoding = ENC_GZIP;
                break;
            case 'i':
            case 'I':
        ...
        if (hstat->local_encoding == ENC_INVALID) {
            ...
            if (type) {
                p = strchr(type, '/');
                if (p == NULL) {
                    hstat->remote_encoding = ENC_GZIP;
                    hstat->local_encoding = ENC_NONE;
                } else {
                    p++;
            ...
            if (hstat->remote_encoding == ENC_GZIP
                && (p = strrchr(u->file, '.'))
    ...
    if (H_REDIRECTED (statcode) || statcode == HTTP_STATUS_MULTIPLE_CHOICES) {
        ...
        if (statcode == HTTP_STATUS_MULTIPLE_CHOICES && !hstat->newloc)
            *dt |= RETROKF;
        else {
            logprintf(LOG_VERBOSE,
                      _("Location: %s%s\n"),
                      hstat->newloc ? escnonprint_uri(hstat->newloc) : _("unspecified"),
                      hstat->newloc ? _(" [following]") : "");
            ...
            /* Normally we are not interested in the response body of a redirect.
               But if we are writing a WARC file we are: we like to keep everything.  */
            if (warc_enabled) {
                int _err = read_response_body(hstat, sock, NULL, contlen, 0,
                                              chunked_transfer_encoding,
                                              u->url, warc_timestamp_str,
                                              warc_request_uuid, warc_ip, type,
                                              statcode, head);
    ...
    if (global_options.adjust_extension) {
        ...
        switch (hstat->local_encoding) {
            ...
            default:
                DEBUGP (("No extension found for encoding %d\n",
                        hstat->local_encoding));
        }
        if (encoding_ext != NULL) {
            char *file_ext = strrchr(hstat->local_file, '.');
        ...
        if (*dt & TEXTHTML)
            /* -E / --adjust-extension / adjust_extension = on was specified,
               ...
               tack on ".html". */
        {
            ensure_extension(hstat, ".html", dt);
        } else if (*dt & TEXTCSS) {
            ensure_extension(hstat, ".css", dt);
        }
        if (encoding_ext != NULL) {
            ensure_extension(hstat, encoding_ext, dt);
        }
    }

    if (cond_get) {
        /* Handle the case when server ignores If-Modified-Since header.  */
        if (statcode == HTTP_STATUS_OK && hstat->remote_time) {
            time_t tmr = http_atotm(hstat->remote_time);

            /* Check if the local file is up-to-date based on Last-Modified header
               and content length.  */
            if (tmr != (time_t) -1 && tmr <= hstat->orig_file_tstamp
                && (contlen == -1 || contlen == hstat->orig_file_size)) {
                logprintf(LOG_VERBOSE,
    ...
    if (statcode == HTTP_STATUS_RANGE_NOT_SATISFIABLE
        && hstat->restval < (contlen + contrange)) {
...
\n    The file is already fully retrieved; nothing to do.\n\n"));
    ...
    if ((contrange != 0 && contrange != hstat->restval)
        || (H_PARTIAL (statcode) && !contrange && hstat->restval)) {
    ...
    else if (hstat->remote_encoding == ENC_GZIP)
        hstat->contlen = -1;
    else
    ...
    err = open_output_stream(hstat, count, &fp);
    if (err != RETROK) {
        CLOSE_INVALIDATE (sock);
        retval = err;
        goto cleanup;
    ...
    err = read_response_body(hstat, sock, fp, contlen, contrange,
                             chunked_transfer_encoding,
                             u->url, warc_timestamp_str,
                             warc_request_uuid, warc_ip, type,
                             statcode, head);
...
http_loop(const struct url *u, struct url *original_url, char **newloc,
          ...
          struct iri *iri) {
    fprintf(stdout, _("http_loop() start\n"));
    fprintf(stdout, _("http_loop() u->port: %d\n"), u->port);
    fprintf(stdout, _("http_loop() u->url: %s\n"), u->url);
    fprintf(stdout, _("http_loop() u->host: %s\n"), u->host);
    fprintf(stdout, _("http_loop() u->path: %s\n"), u->path);
    fprintf(stdout, _("http_loop() u->params: %s\n"), u->params);
    fprintf(stdout, _("http_loop() u->query: %s\n"), u->query);
    fprintf(stdout, _("http_loop() u->fragment: %s\n"), u->fragment);
    fprintf(stdout, _("http_loop() *dt: %d\n"), *dt);

    fprintf(stdout, _("http_loop() *newloc: %s\n"), *newloc);
    fprintf(stdout, _("http_loop() *local_file: %s\n"), *local_file);
    fprintf(stdout, _("http_loop() referer: %s\n"), referer);
    ...
    *dt = 0;
    ...
    do {
        ...
        fprintf(stdout, _("http_loop() hstat.newloc before: %s\n"), hstat.newloc);// NULL
        ...
        fprintf(stdout, _("http_loop() hstat.newloc after: %s\n"), hstat.newloc);
        if (hstat.newloc)
...
Remote file exists.\n\n"));
    ...
    free_hstat(&hstat);
    fprintf(stdout, _("http_loop() end\n"));
    return ret;
}
