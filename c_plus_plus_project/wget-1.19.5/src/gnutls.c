...
ssl_init(void) {
    ...
    if (!global_options.ca_directory) {
        ncerts = gnutls_certificate_set_x509_system_trust(credentials);
        fprintf(stdout, _("ssl_init() ncerts: %d\n"), ncerts);
    }
#endif
    ...
    if (ncerts <= 0) {
        ca_directory = global_options.ca_directory ? global_options.ca_directory : "/etc/ssl/certs";
        if ((dir = opendir(ca_directory)) == NULL) {
            if (global_options.ca_directory && *global_options.ca_directory)
                logprintf(LOG_NOTQUIET, _("ERROR: Cannot open directory %s.\n"),
                          global_options.ca_directory);
        } else {
    ...
    fprintf(stdout, _("ssl_init() global_options.ca_cert: %d\n"), global_options.ca_cert);
    if (global_options.ca_cert) {
        int rc;

        ncerts = 0;

        if ((rc = gnutls_certificate_set_x509_trust_file(credentials, global_options.ca_cert,
                                                         GNUTLS_X509_FMT_PEM)) <= 0)
            logprintf(LOG_NOTQUIET, _ ("ERROR: Failed to open cert %s: (%d).\n"),
                      global_options.ca_cert, rc);
        else {
            ncerts += rc;
    ...
    fprintf(stdout, _("ssl_init() global_options.crl_file: %s\n"), global_options.crl_file);
    if (global_options.crl_file) {
        int rc;

        if ((rc = gnutls_certificate_set_x509_crl_file(credentials, global_options.crl_file, GNUTLS_X509_FMT_PEM)) <=
            0) {
            logprintf(LOG_NOTQUIET, _("ERROR: Failed to load CRL file '%s': (%d)\n"), global_options.crl_file, rc);
            return false;
        }
    ...
    fprintf(stdout, _("ssl_init() global_options.cert_file: %s\n"), global_options.cert_file);
    fprintf(stdout, _("ssl_init() global_options.private_key: %s\n"), global_options.private_key);
    ...
    if (global_options.cert_file && global_options.private_key) {
        int type;
        if (global_options.private_key_type != global_options.cert_type) {
            /* GnuTLS can't handle this */
            logprintf(LOG_NOTQUIET, _("ERROR: GnuTLS requires the key and the \
cert to be of the same type.\n"));
        }

        type = key_type_to_gnutls_type(global_options.private_key_type);

        gnutls_certificate_set_x509_key_file(credentials, global_options.cert_file,
                                             global_options.private_key,
                                             type);
    }
...
wgnutls_read(int fd, char *buf, int bufsize, void *arg) {
    ...
    ret = wgnutls_read_timeout(fd, buf, bufsize, arg, global_options.read_timeout);
    if (ret < 0)
        ctx->last_error = ret;
...
wgnutls_peek(int fd, char *buf, int bufsize, void *arg) {
    ...
    if (bufsize > offset) {
        if (global_options.read_timeout && gnutls_record_check_pending(ctx->session) == 0
            && select_fd(fd, 0.0, WAIT_FOR_READ) <= 0)
            read = 0;
        else
            read = wgnutls_read_timeout(fd, buf + offset, bufsize - offset,
                                        ctx, global_options.read_timeout);
        if (read < 0) {
            if (offset)
                read = 0;
            else
    ...
    switch (global_options.secure_protocol) {
        ...
        default:
            logprintf(LOG_NOTQUIET, _("GnuTLS: unimplemented 'secure-protocol' option value %u\n"),
                      (unsigned) global_options.secure_protocol);
...
# define FD_TO_SOCKET(X) (X)
...
#if HAVE_GNUTLS_PRIORITY_SET_DIRECT
        err = gnutls_priority_set_direct(session, global_options.tls_ciphers_string, NULL);
#else
    ...
    if (continue_session) {
        ...
        if (!gnutls_session_is_resumed(session)) {
            if (!ctx || !ctx->session_data ||
                gnutls_session_set_data(session, ctx->session_data->data, ctx->session_data->size)) {
                if (ctx && ctx->session_data) {
    ...
    err = _do_handshake(session, fd, global_options.connect_timeout);
...
ssl_check_certificate(int fd, const char *host) {
    ...
    _CHECK_CERT (GNUTLS_CERT_INSECURE_ALGORITHM,
                 _("%s: The certificate of %s was signed using an insecure algorithm.\n"));
