...
static int
realloc_texture(SDL_Texture **texture, Uint32 new_format, int new_width,
...
static void video_image_display(VideoState *is) {
    ...
    vp = frame_queue_peek_last(&is->pictFQ);
    if (is->subtitle_st) {
        if (frame_queue_nb_remaining(&is->subpFQ) > 0) {
            sp = frame_queue_peek(&is->subpFQ);
...
static void video_audio_display(VideoState *s) {
    ...
    if (!s->paused) {
        ...
        if (audio_callback_time) {
            ...
            delay -= (time_diff * s->audio_tgt.sample_rate) / 1000000;
        }
    ...
    if (s->show_mode == SHOW_MODE_WAVES) {
        ...
        /*if (realloc_texture(vis_texture, SDL_PIXELFORMAT_ARGB8888,
...
static double vp_duration(VideoState *is, Frame *vp, Frame *nextvp) {
    if (vp->serial == nextvp->serial) {
        ...
        //printf("vp_duration() duration: %lf\n", duration);
...
static void update_video_pts(VideoState *is, double pts, int64_t pos, int serial) {
    ...
    set_clock(&is->vidClock, pts, serial);
...
static void video_refresh(void *opaque, double *remaining_time) {
    ...
    if (is->video_st) {
        retry:
        if (frame_queue_nb_remaining(&is->pictFQ) == 0) {
            // nothing to do, no picture to display in the queue
        } else {
            double last_duration, duration, delay;
            // vp:当前Frame lstvp:上一个Frame
            Frame *vp, *lastvp;
            ...
            if (frame_queue_nb_remaining(&is->pictFQ) > 1) {
                Frame *nextvp = frame_queue_peek_next(&is->pictFQ);
                /*printf("video_refresh() relative_time: %lf, last_relative_time: %lf, next_relative_time: %lf\n",
                       vp->relative_time, lastvp->relative_time, nextvp->relative_time);*/
            ...
            if (is->subtitle_st) {
                ...
                while (frame_queue_nb_remaining(&is->subpFQ) > 0) {
                    sp = frame_queue_peek(&is->subpFQ);

                    if (frame_queue_nb_remaining(&is->subpFQ) > 1)
                        sp2 = frame_queue_peek_next(&is->subpFQ);
                    else
                        sp2 = NULL;

                    if (sp->serial != is->subtitlePQ.serial
                        || (is->vidClock.pts > (sp->pts + ((float) sp->sub.end_display_time / 1000)))
                        || (sp2 && is->vidClock.pts > (sp2->pts + ((float) sp2->sub.start_display_time / 1000)))) {
                        if (sp->uploaded) {
                            int i;
                            for (i = 0; i < sp->sub.num_rects; i++) {
                                AVSubtitleRect *sub_rect = sp->sub.rects[i];
    ...
    if (show_status) {
        ...
        if (!last_time || (cur_time - last_time) >= 30000) {
            ...
            sqsize = 0;
            if (is->audio_st)
                aqsize = is->audioPQ.size;
            if (is->video_st)
                vqsize = is->videoPQ.size;
            if (is->subtitle_st)
                sqsize = is->subtitlePQ.size;
            av_diff = 0;
            if (is->audio_st && is->video_st)
                av_diff = get_clock(&is->audClock) - get_clock(&is->vidClock);
            else if (is->video_st)
            ...
            av_log(NULL, AV_LOG_INFO,
                   ...
                   vqsize / 1024,
                   sqsize,
                   is->video_st ? is->vidDecoder.avctx->pts_correction_num_faulty_dts : 0,
                   is->video_st ? is->vidDecoder.avctx->pts_correction_num_faulty_pts : 0);
            fflush(stdout);
            last_time = cur_time;
        }
    }
...
static int queue_picture(VideoState *is, AVFrame *decodedAVFrame, double pts,
    ...
    Frame *vp;
    if (!(vp = frame_queue_peek_writable(&is->pictFQ))) {
        return -1;
    }
...
static int configure_video_filters(AVFilterGraph *graph, VideoState *is, const char *vfilters, AVFrame *frame) {
    ...
    AVRational fr = av_guess_frame_rate(is->avFormatContext, is->video_st, NULL);
        ...
        if (got_frame) {
            ...
            reconfigure =
                    cmp_audio_fmts(is->audio_filter_src.sample_fmt, is->audio_filter_src.channels,
                                   (enum AVSampleFormat) decodedAVFrame->format, decodedAVFrame->channels)
            ...
            if (reconfigure) {
                ...
                av_log(NULL, AV_LOG_DEBUG,
                       ...
                       av_get_sample_fmt_name((enum AVSampleFormat) decodedAVFrame->format), buf2,
                       is->audDecoder.pkt_serial);

                is->audio_filter_src.sample_fmt = (enum AVSampleFormat) decodedAVFrame->format;
...
static void *video_thread(void *arg) {
    ...
    AVRational frame_rate = av_guess_frame_rate(is->avFormatContext, is->video_st, NULL);
    ...
    int last_w = 0;
    int last_h = 0;
    enum AVPixelFormat last_format = (enum AVPixelFormat) -1;// -2
    int last_serial = -1;
    ...
    for (;;) {
        ...
        if (last_w != decodedAVFrame->width
            ...
            av_log(NULL, AV_LOG_DEBUG,
                   ...
                   (const char *) av_x_if_null(av_get_pix_fmt_name((enum AVPixelFormat) decodedAVFrame->format),
                                               "none"),
            ...
            last_h = decodedAVFrame->height;
            last_format = (enum AVPixelFormat) decodedAVFrame->format;
            last_serial = is->vidDecoder.pkt_serial;
            last_vfilter_idx = is->vfilter_idx;
            frame_rate = av_buffersink_get_frame_rate(filt_out);
        }

        ret = av_buffersrc_add_frame(filt_in, decodedAVFrame);
        if (ret < 0)
            goto the_end;
...
static void *subtitle_thread(void *arg) {
    ...
    for (;;) {
        if (!(sp = frame_queue_peek_writable(&is->subpFQ)))
            return 0;

        if ((got_subtitle = decoder_decode_frame(&is->subDecoder, NULL, &sp->sub)) < 0)
            break;
        ...
        if (got_subtitle && sp->sub.format == 0) {
            ...
            sp->pts = pts;
            sp->serial = is->subDecoder.pkt_serial;
            sp->width = is->subDecoder.avctx->width;
            sp->height = is->subDecoder.avctx->height;
            sp->uploaded = 0;

            /* now we can update the picture count */
            frame_queue_push(&is->subpFQ);
...
static int audio_decode_frame(VideoState *is) {
    ...
    if (af->frame->format != is->audio_src.sample_fmt ||
        ...
        is->audio_src.sample_fmt = (enum AVSampleFormat) af->frame->format;
    }
...
static int stream_component_open(VideoState *is, int stream_index) {
    ...
    switch (codecContext->codec_type) {
        ...
        case AVMEDIA_TYPE_VIDEO: {
            ...
            if ((ret = decoder_start(&is->vidDecoder, video_thread, "video_decoder", is)) < 0)
                goto out;
        ...
        case AVMEDIA_TYPE_SUBTITLE: {
            ...
            if ((ret = decoder_start(&is->subDecoder, subtitle_thread, "subtitle_decoder", is)) < 0)
                goto out;
            break;
        }
        default:
    ...
    for (i = 0; i < AVMEDIA_TYPE_NB; i++) {
        if (wanted_stream_spec[i] && stream_index[i] == -1) {
            av_log(NULL, AV_LOG_ERROR, "Stream specifier %s does not match any %s stream\n",
                   ...
                   av_get_media_type_string((enum AVMediaType) i));
    ...
    for (;;) {
        ...
        if (is->seek_req) {
            ...
            if (ret < 0) {
                ...
                if (is->subtitle_stream >= 0) {
                    packet_queue_flush(&is->subtitlePQ);
                    packet_queue_put(&is->subtitlePQ, &flush_pkt);
                }
                if (is->video_stream >= 0) {
                    packet_queue_flush(&is->videoPQ);
        ...
        if (infinite_buffer < 1 &&
            (is->audioPQ.size + is->videoPQ.size + is->subtitlePQ.size > MAX_QUEUE_SIZE
             || (stream_has_enough_packets(is->audio_st, is->audio_stream, &is->audioPQ) &&
                 stream_has_enough_packets(is->video_st, is->video_stream, &is->videoPQ) &&
                 stream_has_enough_packets(is->subtitle_st, is->subtitle_stream, &is->subtitlePQ)))) {
            /* wait 10 ms */
        ...
        if (!is->paused
            ...
            && (!is->video_st
                || (is->vidDecoder.finished == is->videoPQ.serial && frame_queue_nb_remaining(&is->pictFQ) == 0))) {
            if (loop != 1 && (!loop || --loop)) {
        ...
        if (ret < 0) {
            if ((ret == AVERROR_EOF || avio_feof(avFormatContext->pb))
                ...
                if (is->subtitle_stream >= 0)
                    packet_queue_put_nullpacket(&is->subtitlePQ, is->subtitle_stream);
                is->is_eof = 1;
            }
        ...
        if (pkt->stream_index == is->audio_stream
            ...
            // 视频帧
            packet_queue_put(&is->videoPQ, pkt);
        } else if (pkt->stream_index == is->subtitle_stream
                   && pkt_in_play_range) {
            // 字幕帧
            packet_queue_put(&is->subtitlePQ, pkt);
        } else {
            av_packet_unref(pkt);
        }
    }// for (;;) end
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat) {
    ...
    /* start video display */
    if (packet_queue_init(&videoState->videoPQ) < 0
        || packet_queue_init(&videoState->audioPQ) < 0
        || packet_queue_init(&videoState->subtitlePQ) < 0) {
        goto fail;
    }

    if ((frame_queue_init(&videoState->pictFQ, &videoState->videoPQ, PICTURE_QUEUE_SIZE, 1) < 0)
        || (frame_queue_init(&videoState->sampFQ, &videoState->audioPQ, SAMPLE_QUEUE_SIZE, 1) < 0)
        || (frame_queue_init(&videoState->subpFQ, &videoState->subtitlePQ, SUBPICTURE_QUEUE_SIZE, 0) < 0)) {
        goto fail;
    }
    ...
    init_clock(&videoState->vidClock, &videoState->videoPQ.serial);
...
static void toggle_audio_display(VideoState *is) {
    enum ShowMode next = is->show_mode;
    do {
        next = (enum ShowMode) ((next + 1) % SHOW_MODE_NB);
