...
void initSwr() {
    if (dst_avformat_context->streams[0]->codec->channels !=
        src_avformat_context->streams[audio_stream_index]->codec->channels
        || dst_avformat_context->streams[0]->codec->sample_rate !=
           src_avformat_context->streams[audio_stream_index]->codec->sample_rate
        || dst_avformat_context->streams[0]->codec->sample_fmt !=
           src_avformat_context->streams[audio_stream_index]->codec->sample_fmt) {
        if (NULL == audio_swr_context) {
            audio_swr_context = swr_alloc();
        }
#if LIBSWRESAMPLE_VERSION_MINOR >= 17    // 根据版本不同，选用适当函数
        av_opt_set_int(pSwrCtx, "ich", src_avformat_context->streams[audio_index]->codec->channels, 0);
        av_opt_set_int(pSwrCtx, "och", dst_avformat_context->streams[0]->codec->channels, 0);
        av_opt_set_int(pSwrCtx, "in_sample_rate",  src_avformat_context->streams[audio_index]->codec->sample_rate, 0);
        av_opt_set_int(pSwrCtx, "out_sample_rate",  dst_avformat_context->streams[0]->codec->sample_rate, 0);
        av_opt_set_sample_fmt(pSwrCtx, "in_sample_fmt", src_avformat_context->streams[audio_index]->codec->sample_fmt, 0);
        av_opt_set_sample_fmt(pSwrCtx, "out_sample_fmt", dst_avformat_context->streams[0]->codec->sample_fmt, 0);
#else
        audio_swr_context =
                swr_alloc_set_opts(audio_swr_context,
                                   dst_avformat_context->streams[0]->codec->channel_layout,
                                   dst_avformat_context->streams[0]->codec->sample_fmt,
                                   dst_avformat_context->streams[0]->codec->sample_rate,
                                   src_avformat_context->streams[audio_stream_index]->codec->channel_layout,
                                   src_avformat_context->streams[audio_stream_index]->codec->sample_fmt,
                                   src_avformat_context->streams[audio_stream_index]->codec->sample_rate,
                                   0, NULL);
#endif
...
int TransSample(AVFrame *in_frame, AVFrame *out_frame) {
    ...
    if (audio_swr_context != NULL) {
        out_frame->nb_samples = av_rescale_rnd(
                swr_get_delay(audio_swr_context, dst_avformat_context->streams[0]->codec->sample_rate) + src_nb_samples,
                dst_avformat_context->streams[0]->codec->sample_rate,
                src_avformat_context->streams[audio_stream_index]->codec->sample_rate,
                AV_ROUND_UP);
        ...
        ret = av_samples_alloc(out_frame->data,
                               ...
                               dst_avformat_context->streams[0]->codec->channels,
                               out_frame->nb_samples,
                               dst_avformat_context->streams[0]->codec->sample_fmt, 0);
        ...
        setup_array(m_ain, in_frame, src_avformat_context->streams[audio_stream_index]->codec->sample_fmt,
                    src_nb_samples);

        //注意这里，out_count和in_count是samples单位，不是byte
        //所以这样av_get_bytes_per_sample(src_avformat_context->streams[audio_index]->codec->sample_fmt) * src_nb_samples是错的
        len = swr_convert(audio_swr_context, out_frame->data, out_frame->nb_samples, (const uint8_t **) m_ain,
                          src_nb_samples);
        if (len < 0) {
...
int flush_encoder2(AVFormatContext *fmt_ctx, unsigned int stream_index) {
    ...
    while (1) {
        ...
        ret = avcodec_encode_audio2(dst_avformat_context->streams[stream_index]->codec, &enc_pkt, NULL, &got_frame);
        if (ret < 0)
            break;
        ...
        enc_pkt.dts = av_rescale_q_rnd(enc_pkt.dts, dst_avformat_context->streams[stream_index]->codec->time_base,
                                       dst_avformat_context->streams[stream_index]->time_base,
                                       (AVRounding) (AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));

        enc_pkt.pts = av_rescale_q_rnd(enc_pkt.pts, dst_avformat_context->streams[stream_index]->codec->time_base,
                                       dst_avformat_context->streams[stream_index]->time_base,
                                       (AVRounding) (AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));

        enc_pkt.duration = av_rescale_q(enc_pkt.duration, dst_avformat_context->streams[stream_index]->codec->time_base,
                                        dst_avformat_context->streams[stream_index]->time_base);

        /* mux encoded frame */
        ret = av_interleaved_write_frame(dst_avformat_context, &enc_pkt);
