//
// Created by weidi5858258 on 2018/1/1.
//

#ifndef GRAMMAR_METHOD_H
#define GRAMMAR_METHOD_H

#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <wchar.h>
#include <string.h>
#include <errno.h>
#include <time.h>

#define PI 3.14159265

struct {
    char name[40];
    int age;
}person, person_copy;

class Method {
public:
    void wd_sin(void);
    void wd_cos(void);
    void wd_tan(void);

    void wd_fgetc(void);
    void wd_fgets(void);
    void wd_fputc(void);
    void wd_fputs(void);
    void wd_getc(void);
    void wd_getchar(void);
    void wd_gets(void);
    void wd_putc(void);
    void wd_putchar(void);
    void wd_puts(void);
    void wd_ungetc(void);

    void wd_fopen(void);
    void wd_fclose(void);
    void wd_fflush(void);

    void wd_memcpy(void);
    void wd_memmove(void);
    void wd_memchr(void);
    void wd_memcmp(void);
    void wd_memset(void);
    void wd_strcat(void);
    void wd_strncat(void);
    void wd_strncat2(void);
    void wd_strcmp(void);
    void wd_strncmp(void);
    void wd_strcpy(void);
    void wd_strncpy(void);
    void wd_strchr(void);
    void wd_strcoll(void);
    void wd_strcspn(void);
    void wd_strlen(void);
    void wd_strpbrk(void);
    void wd_strerror(void);

    void wd_fgetwc(void);

    void wd_abort(void);
    void wd_abs(void);
    void wd_atexit(void);
    void wd_atof(void);
    void wd_atoi(void);
    void wd_atol(void);
    void wd_atoll(void);
    void wd_at_quick_exit(void);
    void wd_bsearch(void);
    void wd_div(void);
    void wd_exit(void);
    void wd_free(void);
    void wd_getenv(void);
    void wd_labs(void);
    void wd_ldiv(void);
    void wd_llabs(void);
    void wd_lldiv(void);
    void wd_malloc(void);
    void wd_calloc(void);
    void wd_realloc(void);
    void wd_mblen(void);
    void wd_mbstowcs(void);
    void wd_mbtowc(void);
    void wd_qsort(void);
    void wd_quick_exit(void);
    void wd_rand(void);
    void wd_srand(void);
    void wd_strtod(void);
    void wd_strtof(void);
    void wd_strtol(void);
    void wd_strtold(void);
    void wd_strtoll(void);
    void wd_strtoul(void);
    void wd_strtoull(void);
    void wd_system(void);
    void wd_wcstombs(void);
    void wd_wctomb(void);
    void wd__Exit(void);
};


#endif //GRAMMAR_METHOD_H
